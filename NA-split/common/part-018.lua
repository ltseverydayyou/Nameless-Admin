...
NAmanage.ModelESP_RemoveRule = function(term, mode)
	term = Lower(tostring(term or "")):gsub("^%s+", ""):gsub("%s+$", "")
	if term == "" then
		return false
	end
	if mode ~= nil then mode = Lower(tostring(mode)) end
	const rules = NAmanage.ModelESP_EnsureRules()
	local removed = false
	for i = #rules, 1, -1 do
		const rule = rules[i]
		if type(rule) == "table" and rule.term == term and (mode == nil or rule.mode == mode) then
