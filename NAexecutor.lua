...
local cg = (function()
	local S = function(_, serviceName)
		return __lt.gs(serviceName)
	end
...
local function pickPar()
	if __NAUIProtector and type(__NAUIProtector.resolveParent) == "function" then
		local ok, parent = pcall(__NAUIProtector.resolveParent, { coreGui = cg })
...
local function AttachEditor(cfg)
				...
				if preEntry then
					...
					elseif string.find(char, "%d") then
						...
						if
							...
							and string.find(string.sub(lineText, endPos + 2, endPos + 2), "%d")
						then
							endPos = endPos + 1
						end
						currentType = 6
	...
	local function updateCanvas()
		...
		for line in (sTabs .. "\n"):gmatch("([^\n]*)\n") do
			...
			local bounds =
				__lt.cm("TextService", "GetTextSize", l, src.TextSize, src.Font, Vector2.new(1000000, lineHeight))
	...
	src.FocusLost:Connect(function() end);
...
local ts, uis, gsv, txt, rs, hs =
	S("TweenService"), S("UserInputService"), S("GuiService"), S("TextService"), S("RunService"), S("HttpService")
...
local fsOk = typeof(isfile) == "function"
