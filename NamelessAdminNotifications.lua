...
local tw, rs, uis, plrs, cg, hs, ts, lg =
	S("TweenService"),
	S("RunService"),
	S("UserInputService"),
	S("Players"),
...
local function mkIcn(par, txt, z, font, stl)
	...
	local function hov(on)
		...
		local t = b:GetAttribute("_stl") or ""
		if t == "bad" then
			tw:Create(b, TweenInfo.new(0.12), { BackgroundColor3 = on and TH.Bad1 or TH.Bad0 }):Play()
		else
			tw:Create(b, TweenInfo.new(0.12), { BackgroundColor3 = on and TH.Btn1 or TH.Btn0 }):Play()
		end
	end
	...
	b.MouseButton1Down:Connect(function()
		...
		local t = b:GetAttribute("_stl") or ""
		tw:Create(b, TweenInfo.new(0.08), { BackgroundColor3 = (t == "bad") and TH.Bad0 or TH.Btn2 }):Play()
	end)
	b.MouseButton1Up:Connect(function()
		local bc, bt = col()
		tw:Create(b, TweenInfo.new(0.10), { BackgroundColor3 = bc, BackgroundTransparency = bt }):Play()
	end)
...
local function mkMenuBtn(par, txt, z, font)
	...
	b.MouseEnter:Connect(function()
		tw:Create(b, TweenInfo.new(0.12), { BackgroundColor3 = TH.Btn1 }):Play()
	end)
	b.MouseLeave:Connect(function()
		tw:Create(b, TweenInfo.new(0.12), { BackgroundColor3 = TH.Btn0 }):Play()
	end)
	...
	local function openFont()
		...
		for _, pair in ipairs(FONTS) do
			...
			local setSel = function(on)
				b:SetAttribute("sel", on and true or false)
				tw:Create(b, TweenInfo.new(0.10), { BackgroundColor3 = on and TH.Ok1 or TH.Btn0, BackgroundTransparency = on and 0.06 or 0.08 }):Play()
			end
			table.insert(rec, { btn = b, font = pair[2], set = setSel })
		...
		tw:Create(sc, TweenInfo.new(0.16, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Scale = 1 }):Play()
		tw:Create(m, TweenInfo.new(0.16, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { BackgroundTransparency = 0.06 }):Play()
	end
...
local function attachTimer(card, fil, dur)
	...
	local function playFrom(f)
		...
		tn = tw:Create(fil, TweenInfo.new(d, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), { Size = UDim2.new(0, 0, 1, 0) })
...
local function appear(card, sc, st, sh, tgt, from, cnt)
	...
	tw:Create(sc, tA, { Scale = 1.05 }):Play()
	task.delay(0.16, function()
		tw:Create(sc, tB, { Scale = 1 }):Play()
	end)

	tw:Create(st, TweenInfo.new(0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { Transparency = 0.55, Thickness = 1 }):Play()
	if sh then
		tw:Create(sh, TweenInfo.new(0.22, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), { ImageTransparency = 0.30 }):Play()
	end
...
local function disappear(card, sc, st, sh)
	local t1 = TweenInfo.new(0.10, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
	local t2 = TweenInfo.new(0.16, Enum.EasingStyle.Sine, Enum.EasingDirection.In)
	tw:Create(sc, t1, { Scale = 1.02 }):Play()
	task.delay(0.07, function()
		tw:Create(sc, t2, { Scale = 0.88 }):Play()
		tw:Create(card, t2, { BackgroundTransparency = 1, Rotation = -1.5 }):Play()
		if st then
			tw:Create(st, t2, { Transparency = 1 }):Play()
		end
		if sh then
			tw:Create(sh, t2, { ImageTransparency = 1 }):Play()
		end
...
local function build(kind, p)
	...
	if p.InputField then
		...
		inp.Focused:Connect(function()
			tw:Create(inp, TweenInfo.new(0.12), { BackgroundColor3 = TH.Btn1 }):Play()
		end)
		inp.FocusLost:Connect(function()
			tw:Create(inp, TweenInfo.new(0.12), { BackgroundColor3 = TH.Btn0 }):Play()
		end)
	end
