...
local tw, rs, uis, plrs, cg, hs, ts, gs =
    S("TweenService"),
    S("RunService"),
    S("UserInputService"),
    S("Players"),
...
(gui:GetPropertyChangedSignal("AbsoluteSize")):Connect(
    function()
...
local function trackPopupCenter(root2, card)
    ...
    ref(true)
    local c1 =
        (card:GetPropertyChangedSignal("AbsoluteSize")):Connect(
        function()
            ref()
        end
    )
    ...
    if cam then
        local c2 =
            (cam:GetPropertyChangedSignal("ViewportSize")):Connect(
            function()
                ref()
            end
        )
...
local function mkHdr(par, z, kind, onPause)
    ...
    local c4 =
        fbtn.MouseButton1Click:Connect(
        function()
            if state.closing then
                return
            end
    ...
    if posBtn then
        ...
        local c6 =
            posBtn.MouseButton1Click:Connect(
            function()
                if state.closing then
                    return
                end
...
local function mkBtnArea(cnt, list, owner, z, maxH, font)
    ...
    local c1 =
        (sf:GetPropertyChangedSignal("AbsoluteSize")):Connect(
        function()
            fitCells()
        end
    )
    ...
    task.defer(
        function()
            ...
            if so and so.sc and so.sc.GetPropertyChangedSignal then
                local c =
                    so.sc:GetPropertyChangedSignal("Scale"):Connect(
                    function()
                        task.defer(fitCells)
                        task.defer(fitH)
                    end
    ...
    for _, info in ipairs(list) do
        ...
        ic.LayoutOrder = 10
        ic.Size =
            hasIcon and UDim2.fromOffset((isMobile and 24 or 22), (isMobile and 24 or 22)) or UDim2.new(0, 0, 0, 0)
        ic.Parent = b
        local lb = Instance.new("TextLabel")
        lb.BackgroundTransparency = 1
        lb.TextColor3 = TH.Txt
...
local function widthForDock(dock, kind)
    ...
    return kind == "Popup" and
        math.floor(
            math.clamp(
                gui.AbsoluteSize.X * (isMobile and 0.88 or 0.36),
                (isMobile and 360 or 340),
                (isMobile and 500 or 620)
            )
        ) or
        nW()
end
            ...
            task.delay(
                ...
                function()
                    local tw5 =
                        tw:Create(
                        card,
                        TweenInfo.new(0.16, Enum.EasingStyle.Sine, Enum.EasingDirection.In),
                        {
...
local function build(kind, p)
    ...
    local timCtl
    local card, hdr, ttl, act, st, sc, cnt, ftr, trk, fil =
        mkCard(
        w,
        baseZ,
        kind,
    ...
    function s.setDock(newDock, persist)
        ...
        local b =
            (newDock == "bottom" or newDock == "bottomLeft" or newDock == "bottomRight") and dirFrom("bottom") or
            dirFrom("top")
        card.Parent = tgt
    ...
    if #btns > 2 then
        ...
        setMSel(false)
        local c1 =
            mBtn.MouseButton1Click:Connect(
            function()
                if s.closing then
                    return
                end
